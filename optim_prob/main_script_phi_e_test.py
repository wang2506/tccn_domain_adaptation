...
def err_calc(psi,chi,chi_init,psi_init,err_type,alpha_init=None,div_flag=False,\
               rads=rad_alld,sqrts=sqrt_alld,hat_ep=hat_ep_alld,\
                div_vals=None,args=args,s_params=hat_w,base_net=start_net,\
                    chi_hat=None,chi_hat_init=None): #ep_mis=ep_mismatch,
    err_denoms = []
    if err_type == 's':
        ...
        for j in range(args.t_devices):
            ...
            for i in range(args.t_devices):
                # build inner denom
                for i_hat in range(args.t_devices):
                    #at a labeled, there exist valid hypothesis
                    if i_hat < args.l_devices and i < args.l_devices: 
                        coeff_hyp_diff = d2d_mismatch_test(c_net_s=base_net,params_t=s_params[i],\
                            params_s=s_params[i_hat],dset=d_train,\
                            indx_t=d_dsets[j],bs=args.div_bs,device=device)
                    else:
                        coeff_hyp_diff = 0 
                    coeff_hyp_diff += 1e-6 #prevent 0 in GP
                    
                    # Q = \psi_{hat{i}} + \frac{hat{x}^T_{hat{i}}}{\alpha_{i,j}\psi_j*emp error}
                    calc_q_init = psi_init[i_hat]+\
                        chi_T_iji_init[i][j][i_hat]/ \
                            (alpha_init[i,j]*psi_init[j]*coeff_hyp_diff)
                    
                    denom_inner = cp.power(psi[i_hat]*calc_q_init/psi_init[i_hat], \
                            psi_init[i_hat]/calc_q_init)
                    denom_inner *= cp.power(chi_T_iji[i][j][i_hat]/  \
                            (alpha[i][j]*psi[j]) * calc_q_init  \
                            / (chi_T_iji_init[i][j][i_hat]/(alpha_init[i][j]*psi_init[j])) \
                    , chi_T_iji_init[i][j][i_hat]/(alpha_init[i][j]*psi_init[j]*coeff_hyp_diff) \
                            /calc_q_init)
                    
                    inner_err_denoms.append(denom_inner)
                    inner_err_con.append(1/denom_inner <= 1)
                
                # build outer denominator 
                if div_flag == False:
                ...
                calc_h_init = chi_init[i][j]/alpha_init[i][j] + \
                    psi_init[i]*cs_factor
                for i_hat in range(args.t_devices):
                    calc_h_init += psi_init[i]*chi_T_iji_init[i][j][i_hat]
            
                denom_outer = cp.power(chi[i][j]/alpha[i][j] * calc_h_init / \
                        (chi_init[i][j]/alpha_init[i][j]),  \
                        (chi_init[i][j]/alpha_init[i][j]) / calc_h_init)
                denom_outer *= cp.power(psi[i]* calc_h_init / \
                        psi_init[i],  \
                        cs_factor*psi_init[i] / calc_h_init)

                for i_hat in range(args.t_devices):
                    t_top = psi[i]*chi_T_iji[i][j][i_hat]*calc_h_init
                    t_bot = psi_init[i]*chi_T_iji_init[i][j][i_hat]
                    denom_outer *= cp.power(t_top/t_bot,t_bot/calc_h_init)

                outer_err_denoms.append(denom_outer)
                # build outer numerator
                outer_err_nums.append(cp.sum(chi_T_iji[i][j])+cs_factor)
            
                # build outer constraint
                outer_err_con.append((cp.sum(chi_T_iji[i][j])+cs_factor)/denom_outer \
                            <= 1)
            
        return inner_err_con,outer_err_con
    else:
...
chi_T_iji = {}
for i in range(args.t_devices):
    chi_T_iji[i] = cp.Variable((args.t_devices,args.t_devices),pos=True)
...
def posy_init(iter_num,cp_vars,args=args):
    ...
    if iter_num == 0:
        ...
        chi_T_iji_init = {}
        for i in range(args.t_devices):
            chi_T_iji_init[i] = 5*np.ones((args.t_devices,args.t_devices))
        ...
        chi_T_iji_init = {}
        for i in range(args.t_devices):
            chi_T_iji_init[i] = cp_vars['chi_T_iji'][i].value

    return psi_init,chi_s_init,chi_t_init,alpha_init,\
        chi_c1,chi_c2,chi_T_iji_init
...
for c_iter in range(args.approx_iters):
    t_dict = {'psi':psi,'chi_s':chi_s,'chi_t':chi_t,'alpha':alpha,\
              'chi_c1':chi_c1,'chi_c2':chi_c2,'chi_T_iji':chi_T_iji}
    psi_init,chi_s_init,chi_t_init,alpha_init,chi_c1_init,chi_c2_init,chi_T_iji_init \
    ...
    if args.div_flag == 0: 
        ...
        inner_cons,outer_cons = err_calc(psi,chi_t,chi_t_init,\
                            psi_init,err_type='t',alpha_init=alpha_init,\
                            div_flag=True,div_vals=div_pairs,\
                            chi_hat=chi_T_iji,chi_hat_init=chi_T_iji_init)
    ...
    posy_con = s_posy_con + inner_cons + outer_cons #+ t_posy_cons
...
if args.div_flag == 1: 
    if args.dset_split == 0:
        if args.dset_type == 'MM':
            ...
            with open(cwd+'/optim_results/obj_val/2NRG_'+str(args.phi_e)+'_'\
                +'devices'+str(args.t_devices)+'_seed'+str(args.seed)\
                +'_'+args.div_nn\
                +'_'+args.dset_type+'_'+args.labels_type,'wb') as f:
                pk.dump(obj_vals,f)
            
            with open(cwd+'/optim_results/psi_val/2NRG_'+str(args.phi_e)+'_'\
                +'devices'+str(args.t_devices)+'_seed'+str(args.seed)\
                +'_'+args.div_nn\
                +'_'+args.dset_type+'_'+args.labels_type,'wb') as f:
                pk.dump(psi_track,f)
            
            with open(cwd+'/optim_results/hat_ep_val/2NRG_'+str(args.phi_e)+'_'\
                +'devices'+str(args.t_devices)+'_seed'+str(args.seed)\
                +'_'+args.div_nn\
                +'_'+args.dset_type+'_'+args.labels_type,'wb') as f:
                pk.dump(hat_ep_alld,f)
            
            with open(cwd+'/optim_results/alpha_val/2NRG_'+str(args.phi_e)+'_'\
                +'devices'+str(args.t_devices)+'_seed'+str(args.seed)\
                +'_'+args.div_nn\
                +'_'+args.dset_type+'_'+args.labels_type,'wb') as f:
                pk.dump(alpha.value,f)
