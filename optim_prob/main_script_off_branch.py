...
if args.init_test != 1:
    ...
    for i in range(args.l_devices):
        ...
        # obtain the source accuracy on the labeled local data #full local dataset
        t_net = deepcopy(ld_nets[i])
        t_net.load_state_dict(params_w)
        acc_i,ce_loss = test_img_strain(t_net,\
                    args.div_bs,d_train,indx=ld_sets[i],device=device)#indx=d_dsets[i],device=device)
        
        if args.eval_err == 0:
            adj_acc_i = acc_i*(len(ld_sets[i])/len(d_dsets[i]))
        elif args.eval_err == 1:
            adj_acc_i = acc_i*(len(ld_sets[i])/len(d_dsets[i])) \
                + 0.5*(len(d_dsets[i])-len(ld_sets[i]))/len(d_dsets[i])
        
        # hat_ep.append((100-acc_i)/100) # need to replace with the final training error
        hat_ep.append((100-adj_acc_i)/100)
        # print(acc_i)
        hat_w[i] = params_w
    ...
    if args.dset_split == 0:
        ...
        with open(cwd+'/source_errors/devices'+str(args.t_devices)+'_seed'+str(args.seed)\
            ...
            'wb') as f:
            pk.dump(hat_w,f)     
else: # for debugging, not used for real runs
    hat_ep = []
    min_ep_vect = 1e-3
    max_ep_vect = 5e-1
...
if args.div_flag == 1: 
    if args.dset_split == 0:
        with open(cwd+'/optim_results/obj_val/'+pre_str+'NRG_'\
            +'devices'+str(args.t_devices)+'_seed'+str(args.seed)\
            +'_'+args.div_nn\
            +'_'+args.dset_type+'_'+args.labels_type,'wb') as f:
            pk.dump(obj_vals,f)
        
        with open(cwd+'/optim_results/psi_val/'+pre_str+'NRG_'\
            +'devices'+str(args.t_devices)+'_seed'+str(args.seed)\
            +'_'+args.div_nn\
            +'_'+args.dset_type+'_'+args.labels_type,'wb') as f:
            pk.dump(psi_track,f)
        
        with open(cwd+'/optim_results/hat_ep_val/'+pre_str+'NRG_'\
            +'devices'+str(args.t_devices)+'_seed'+str(args.seed)\
            +'_'+args.div_nn\
            +'_'+args.dset_type+'_'+args.labels_type,'wb') as f:
            pk.dump(hat_ep_alld,f)
        
        with open(cwd+'/optim_results/alpha_val/'+pre_str+'NRG_'\
            +'devices'+str(args.t_devices)+'_seed'+str(args.seed)\
            +'_'+args.div_nn\
            +'_'+args.dset_type+'_'+args.labels_type,'wb') as f:
