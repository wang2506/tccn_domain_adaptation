...
def optim_parser():
    ...
    parser.add_argument('--seed',type=int,default=1) #[1,2,3,4,5] [6,7]
    ...
    parser.add_argument('--phi_s',type=float,default=1,\
                        help='scaling the source errors') #0.1 #old - 1
    parser.add_argument('--phi_t',type=float,default=50*0.01,\
                        help='scaling the target errors') #5 #old - 50
    parser.add_argument('--phi_e',type=float,default=1e0,\
                        help='scaling the energy term') #1e0
    #1e0 for all the ML experiment
    # 1e2,1e0,1e-2 for the alpha bars
    #1e-2 for all the divergence experiments
    
    ## optimization constants
    parser.add_argument('--approx_iters',type=int,default=50,\
                        help='posynomial approximation iterations') #50; 300 for MM+U
    ...
    parser.add_argument('--div_flag',type=int,default=1,\
                        help='estimate divergence yes or no') #1 - yes
    parser.add_argument('--suffix',type=int,default=1,\
                        choices = [1,2], help = 'modified source err yes/no')
    parser.add_argument('--div_style',type=str,default='off',\
                        choices = ['unif','extreme','random','off'], \
                        help='3 divergence cases for the 3x3 plot \n'+\
                        'only active if div_flag == 0')
    parser.add_argument('--test_pe',type=int,default=1,\
                        help='ablation study for phi_e; yes or no') #1 - yes, 0 - no
    
    ## for reviewers [0 means use 1, 1 means use 0.5]
    parser.add_argument('--eval_err',type=int,default=0,\
                        help='use 1 or 0.5 for the err of unlabeled data at sources')
    ...
    parser.add_argument('--div_nn',type=str,default='CNN',\
                        ...
                        help='neural network for divergence estimation') #CNN
