...
from optim_utils.optim_parser import optim_parser,div_ablate_parser

cwd = os.getcwd()
args = optim_parser()
dargs = div_ablate_parser()
...
if dargs.div_ablation == 0: #so default settings
...
if args.div_flag == 1 and dargs.div_ablation == 0: 
    ...
    with open(cwd+'/div_results/ablation_study/ep_hat_vals_unif_maxdiv','wb') as f:
        pk.dump(hat_ep_alld,f)
    
    with open(cwd+'/div_results/ablation_study/alpha_vals_unif_maxdiv','wb') as f:
        pk.dump(alpha.value,f)
    
    with open(cwd+'/div_results/ablation_study/unif_maxdiv_pairs','wb') as f:
        pk.dump(div_pairs,f)
    ...
    with open(cwd+'/div_results/ablation_study/ep_hat_vals_'+dargs.div_hetero,'wb') as f:
        pk.dump(hat_ep_alld,f)
    
    with open(cwd+'/div_results/ablation_study/alpha_vals_'+dargs.div_hetero,'wb') as f:
        pk.dump(alpha.value,f)

    with open(cwd+'/div_results/ablation_study/div_pairs_'+dargs.div_hetero,'wb') as f:
        pk.dump(div_pairs,f)
