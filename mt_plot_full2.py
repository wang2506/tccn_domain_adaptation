...
def get_data(dset_split=dset_split,nn_style=nn_style,nrg=nrg_mt,phi_e=phi_e,grad_rv=grad_rv):
    ...
    return taccs,raccs,h1accs,h2accs,oo_accs
...
taccs,raccs,h1accs,h2accs,oo_accs = get_data(0)
...
for i,j in enumerate(dset_vec):
    ...
    ax[i+3].bar([3],np.mean(h2accs[j]),yerr=np.std(h2accs[j]),ecolor='black',\
              capsize=5,width=width,\
            color='tab:brown',edgecolor='black',label=r'Only-Max-Acc')  
    ax[i+3].bar([4],np.mean(oo_accs[j]),yerr=np.std(oo_accs[j]),ecolor='black',\
              capsize=5,width=width,\
            color='tab:purple',edgecolor='black',label=r'Single Matching')
...
if grad_rv == True:
    ...
    pend = ''
fig.savefig(cwd+'/mt_plots/psi_infocom.pdf',dpi=1000,bbox_inches='tight')
# if dset_split == 0:
