...
# %% extract data
def get_data(dset_split=dset_split,nn_style=nn_style,nrg=nrg_mt,phi_e=phi_e,grad_rv=grad_rv):
    ...
    return taccs,raccs,h1accs,h2accs,om_accs

# %% plot bars for max-min
fig,ax = plt.subplots(1,6,figsize=(8,2),dpi=250,sharey=True)
cats = 5
width = 0.8
...
taccs,raccs,h1accs,h2accs,om_accs = get_data(0)
...
taccs,raccs,h1accs,h2accs,om_accs = get_data(2)
dset_vec = ['M+MM','M+U','MM+U']
for i,j in enumerate(dset_vec):
    ax[i+3].bar([0],np.mean(taccs[j]),yerr=np.std(taccs[j]),ecolor='black',\
             capsize=5,width=width,\
            color='tab:blue',edgecolor='black',label=r'Our Method')
    ax[i+3].bar([1],np.mean(raccs[j]),yerr=np.std(raccs[j]),ecolor='black',\
              capsize=5,width=width,\
            color='tab:orange',edgecolor='black',label=r'Random-$\alpha$')
    ax[i+3].bar([2],np.mean(h1accs[j]),yerr=np.std(h1accs[j]),ecolor='black',\
              capsize=5,width=width,\
            color='tab:green',edgecolor='black',label=r'Qty-Scaled')    
    ax[i+3].bar([3],np.mean(h2accs[j]),yerr=np.std(h2accs[j]),ecolor='black',\
              capsize=5,width=width,\
            color='tab:brown',edgecolor='black',label=r'Uniform')  
    ax[i+3].bar([4],np.mean(om_accs[j]),yerr=np.std(om_accs[j]),ecolor='black',\
              capsize=5,width=width,\
            color='tab:purple',edgecolor='black',label=r'Avg-Degree')
    # if i == 0: 
...
#(x, y, width, height)
leg1 = ax[0].legend(h[:3],l[:3],bbox_to_anchor=(-0.5,1.12,7.6,0.2),\
                        mode='expand',fontsize=10,**kw2) #,**kw2)
leg2 = ax[0].legend(h[3:],l[3:],bbox_to_anchor=(-0.5,0.98,7.6,0.2),\
...
if grad_rv == True:
    ...
    pend = ''
fig.savefig(cwd+'/mt_plots/alpha_infocom.pdf',dpi=1000,bbox_inches='tight')
# if dset_split == 0:
